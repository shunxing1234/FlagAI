...
def get_wds_dataset(data_path, preprocess_img, is_train, 
                    batch_size=32, dataset_resampled=False,
                    epoch=0, floor=False, tokenizer=None):
    input_shards = data_path

    assert input_shards is not None
    resampled = dataset_resampled
...
if __name__ == "__main__":
    ...
    ds = get_wds_dataset("./mscoco/00000.tar", 
                        preprocess_img=transform, 
                        tokenizer=tokenizer, 
                        is_train=True, 
                        epoch=1)

    for samples, target in ds.dataloader :
        print(samples.shape)
        print(target.shape)
        break 